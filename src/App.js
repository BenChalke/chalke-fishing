...
export default function App() {
  ...
  // ————————————————
  //  ON MOUNT: initialize fish, mobile flag, welcome popup
  // ————————————————
  useEffect(() => {
    // 1) Populate initial fish
    setFishArray(createInitialFish());

    // 2) Detect if mobile/touch
    setIsMobile('ontouchstart' in window);

    // 3) Check “seenWelcome” flag
  ...
  // ————————————————
  //  SAVE CAUGHT RECORDS AFTER ANY CHANGE
  // ————————————————
  useEffect(() => {
  ...
  // ————————————————
  //  RESET EVERYTHING (BUT KEEP STATS)
  // ————————————————
  const handleReset = () => {
    setCatchAnimations([]);
    setFishArray(createInitialFish());
  ...
  return (
    ...
    >
      ...
      {/* ————————————————
          3) FISHING HOOK CURSOR (DESKTOP ONLY)
           ———————————————— */}
      ...
      {/* ————————————————
          5) DISPLAY CURRENT SPEED & TOTAL FISH CAUGHT
           ———————————————— */}
      ...
      <button
        ...
        onTouchEnd={toggleCaughtPopup}
      >
        Fish Caught
      </button>

      {/* ————————————————
          7) “FISH CAUGHT” POPUP WITH STATS
           ———————————————— */}
      {showCaughtPopup && (
